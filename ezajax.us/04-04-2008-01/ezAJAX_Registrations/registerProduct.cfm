				...
				if (NOT Request.dbError) {
					...
					if (NOT Request.dbError) {
						...
						if ( (Len(URL.serverName) gt 0) AND (ezIsValidServerName(URL.serverName)) ) { // this allows any server name one might imagine using even IP addresses...
							_sql_statement = "SELECT id, uid, serverName FROM EZAJAX.ServerNames WHERE (uid = #uid#) AND (serverName = '#URL.serverName#')";
											...
											if (Request.qGetLicenseFromDb.recordCount gt 0) {
												...
												if (NOT Request.fileError) {
													...
													if (NOT Request.anError) {
														dataStream = 'Notice 102. Your Runtime License, which was previously created for you may be downloaded directly from "#optionsStruct.cfmailparam.file#" or visit http://ezajax.us to Retrieve your Runtime License online whenever you wish.';	
													} else {
												...
												if (NOT Request.dbError) {
													...
													if (NOT Request.anError) {
														...
														dataStream = 'Error 101. The Runtime License has been assigned to you and you may download it from "#optionsStruct.cfmailparam.file#" or visit http://ezajax.us to Retrieve your Runtime License Online whenever you wish.';
													}
												} else {
